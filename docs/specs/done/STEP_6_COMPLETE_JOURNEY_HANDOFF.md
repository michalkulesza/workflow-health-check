...
| Unit/integration/E2E counts and commands | J10 on 2026-09-14: `npm run typecheck` passed; `npm test` passed (14 files, 36 tests); and `npm run build` passed when given the private preview runtime variables. The four focused integration cases each passed against one fresh task-owned PostgreSQL instance migrated on port 55433, then removed: complete report (14.5s), one clarification (18.5s), notification transport capture (9.9s), and restart/terminal fallback (13.8s). Configured-preview Playwright smoke passed (2 passed, 3 opt-in journeys skipped). `npm run format:check` still reports 132 pre-existing files, and `npm run lint` reports 9,368 existing formatting/lint errors; neither was modified in J10. A normal `npm run build` without a valid local `PAYLOAD_SECRET` fails configuration, while the configured-preview build passes. |
...
| Run/report/lead linkage evidence | The complete-journey test proves the submitted 16-answer snapshot becomes one persisted report through six fresh `worker --once` processes. It asserts project 0/0, people 1/22 normalized, admin 0.05/1, and friction 0.25/5; report evidence cites the submitted Q11 text. Duplicate contact requests create one linked New lead with the exact run ID; its contacted stage and note persist through Payload. J8's fictional live-provider run `18` uses the new published version `4`; it completed in about 20 seconds after one clarification, with schema-valid persisted AI output and a completed narrative report. Its valid no-major-issues outcome has zero priorities. |
| Mobile/desktop/accessibility results | On 2026-09-14, the disposable `https://localhost` Compose preview completed the 16-question desktop journey in 8.8s and 390px mobile journey in 8.2s. Both save, reload, submit, reach results, and submit contact; the desktop case uses keyboard option selection and Next activation. Axe checks pass at form, review, results, and contact. Under the explicit Compose-only clarification transport, the full browser clarification journey passed in 12.3s, including follow-up accessibility, submission, worker continuation, results, and contact. |
...
| Provider-stub vs live Gemini evidence | Test-only Gemini and narrative transports require both `NODE_ENV=test` and `RUN_INTEGRATION_TESTS=true`; no HTTP input can enable them. The passing complete scenario validates grounded AI output and narrative persistence; the clarification scenario persists one prompt/response then completes. Terminal provider failure retries three times and persists a usable partial report without leaving work queued. On 2026-09-14, an authorized local Compose preview completed J8 with fictional data through the real worker and Gemini adapter. The first configured model, `gemini-2.5-flash`, returned an upstream 404 because it is unavailable to new users; a new immutable publication version `4` then pinned the supported stable `gemini-3.6-flash`. Run `18` completed with one clarification, persisted schema-valid rubric output and a grounded narrative, and did not invoke notification/email delivery. The new provider-usage migration was applied to the preview: a further live run `19` persisted Gemini evaluation usage (1,218 total tokens: 497 prompt, 95 output, 626 reasoning). Focused task tests prove the same numeric provenance is retained for the completed narrative. |
| Notification/email scope verified or deferred | J9's isolated integration fixture creates one report-ready notification request before completion and proves it creates zero leads. The real worker enqueues one delivery; an injected transport captures exactly one report-ready email with the expected fictional recipient, subject, sender, private-report path, idempotency key, and persisted report grant/delivery state. No Resend API request or inbox delivery was made. Live sender/domain/inbox verification remains a Step 7 follow-up and requires a designated test recipient. |
...
| Open issues and Step 7 task list | Step 6 functional acceptance is complete. Step 7 follow-up: live report-ready sender/recipient validation and private-link recovery, including any unresolved delayed-clarification behavior. Repository-wide formatting/lint baseline failures remain separate maintenance work. |

### Remaining Step 6 acceptance work

- [x] J1/J3: exact submitted snapshot and publication/run linkage are covered, including a harmless CMS publication change followed by a review edit: the in-progress submission retains its original frozen definition/version while a new submission receives the new version.
...
- [x] J7: configured-preview browser evidence covers the complete 16-question desktop/mobile journeys, keyboard flow, accessible form/review/results/contact, and the isolated provider-backed clarification path.
- [x] J8: an authorized local Compose preview completed live Gemini run `18` against fictional data using publication version `4` and `gemini-3.6-flash`; one clarification and the persisted completed narrative report were verified. After the usage migration, live run `19` persisted numeric Gemini evaluation provenance (1,218 total tokens); focused task tests cover narrative usage persistence.
- [x] J9: a migrated isolated PostgreSQL integration test proves notification-only capture creates zero leads, and the normal worker plus injected transport captures one report-ready delivery and persists its report grant. Live Resend sender/domain/inbox verification remains a Step 7 follow-up for a designated test recipient.
- [x] J10: reviewed commits through `9154b51`, reran the focused provider-controlled integration cases in an isolated migrated database, added version/review-edit coverage and provider usage provenance, and recorded unit/build/browser evidence plus known repository-wide check failures.

Next bounded task: Step 7 live report-ready sender/recipient and private-link recovery verification; separately decide whether to remediate the repository-wide formatting/lint baseline.
