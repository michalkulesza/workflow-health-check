...
| Runtime / npm / image ID | Workstation: Node `v22.20.0`, npm `10.9.3`, Docker `29.7.2`. The supported Node runtime is installed and active. Built `workflow-health-check:preview-local-20260914` from the Dockerfile; image digest `sha256:cab162443ba912e074431d68498c735d731d98317b57adc24bac943312b2b822`. |
| Local fresh-start verification | On 2026-09-14, recreated only the named disposable Compose PostgreSQL volume, started PostgreSQL 16.11 healthy on `127.0.0.1:55432`, applied all 12 migrations, seeded and published questionnaire `5dc13945-9cb8-4e6b-b504-187c885e0e34`, then verified a repeat seed is unchanged. The guarded bootstrap created one administrator; its immediate repeat refused overwrite. |
...
| Compose migration/web/worker proof | On 2026-09-14, a separate `workflow-health-preview` Compose project ran all 12 migrations from the tagged image, then started healthy web and worker services from image digest `sha256:cab162…`; inspection confirmed both roles use that digest. The Caddy proxy returned `{"status":"ok"}` at `https://localhost/api/health/live`. A web/worker restart returned both services to healthy status and preserved proxy health. No Gemini or Resend credentials were supplied. |
...
| Restart / persistence evidence | On 2026-09-14, two separate `npm run worker:once` processes each recorded a worker heartbeat and were followed by a passing `npm run worker:health` check against the fresh disposable database. This verifies heartbeat persistence across worker process restarts; persisted assessment-job recovery remains a later journey check. |
